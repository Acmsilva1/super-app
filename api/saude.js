...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (!membro_familia || !tipo_registro) return json(res, 400, { error: 'membro_familia e tipo_registro obrigatorios' });
    const payload = payloadInsert(membro_familia, tipo_registro, detalhes, data_evento, hora_evento, anexo_url);
